...
- [ ] 16. Ensure `Program.cs` uses the minimal hosting model, registers controllers (`AddControllers`), maps controllers (`MapControllers`), and configures Swagger/OpenAPI with Swashbuckle (`AddEndpointsApiExplorer`, `AddSwaggerGen`, `UseSwagger`, `UseSwaggerUI`). Add the `Swashbuckle.AspNetCore` NuGet package.
